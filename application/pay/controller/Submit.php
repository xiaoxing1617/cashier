    ...
    public function Submit(Request $request)
    {
        ...
        switch ($mod) {
            ...
            case "cashier":
                ...
                if (!array_key_exists('code', $input)) {
                    weuiMsg('warn-primary', '请前往收银台填写金额后再支付');
                    return;
                }
        ...
        if(array_key_exists('third_trade_no',$input) && $mod=='api'){
            if(Order::getByThirdTradeNo($input['third_trade_no'])){
                weuiMsg('warn-primary', '第三方订单号已重复，请重新发起支付！', '警告');
                return;
            }
        }else{
        ...
        if (!array_key_exists('type', $input) or !array_key_exists('money', $input)) {
            weuiMsg('warn-primary', '必须传入支付方式和支付金额', '警告');
            return;
        } else {
        ...
        if ($pay_interface_id == 0 or $pay_interface_id == '') {
            weuiMsg('info', '该商户未开启【' . $client_arr['name'] . '】支付方式', '抱歉');
            return;
        }
        $pay = Pay::getById($pay_interface_id);  //获取接口信息
        if (!$pay) {
            weuiMsg('info', '该商户【' . $client_arr['name'] . '】接口不存在或已被删除', '抱歉');
            return;
        }
        if ($pay['state'] == "0") {
            weuiMsg('info', '当前接口已关闭', '抱歉');
            return;
        }
        if (!$pay->isPayInfo($pay['plug_in'])) {
            weuiMsg('info', '该商户接口信息待完善(' . $pay['plug_in'] . ')', '抱歉');
            return;
        }
        if ($pay['uid'] != $sql['uid']) {
            weuiMsg('warn-primary', '当前接口不属于该商户添加的，请联系该商户及时修改', '警告');
            return;
        }
        //支付限额
        if (!empty($pay['min_money']) && $pay['min_money'] > 0 && $money < $pay['min_money']) {
            weuiMsg('info', '当前接口每次至少支付' . $pay['min_money'] . "元", '抱歉');
            return;
        }
        if (!empty($pay['max_money']) && $pay['max_money'] > 0 && $money > $pay['max_money']) {
            weuiMsg('info', '当前接口每次最多支付' . $pay['max_money'] . "元", '抱歉');
        }
        if($mod == "api"){
        ...
        if($trade_name['code']!=0){
            weuiMsg('warn-primary', $trade_name['msg'], '安全拦截');
            return;
        }
        //=====插件订单====
        $order = [
        ...
        if ($data = $insert->create($order)) {
            ...
            if ($temp->save() === false) {
                ...
                weuiMsg('info', '同步通知地址或异步通知地址设置失败！', '抱歉');
                return;
            }
            //第一次，1分钟，3分钟，10分钟，1小时，2小时，6小时，12小时，最后一次
            $out_trade_no = $data->out_trade_no;
            $url = "/submit/" . $pay['plug_in'] . "/?out_trade_no=" . $out_trade_no;
            $this->assign('url', $url);  //输出变量
            return $this->fetch('submit/index');  //进入模板
        } else {
            weuiMsg('warn-primary', '发起支付失败，请稍后重试', '抱歉');
            return;
        }
    }
