...
class AopCertClient
{
    ...
    public function pageExecute($request, $httpmethod = "POST", $appAuthToken = null) {
        ...
        if (strcasecmp($this->fileCharset, $this->postCharset)) {
            weuiMsg('warn-primary', "文件编码：[" . $this->fileCharset . "] 与表单提交编码：[" . $this->postCharset . "]两者不一致!", '支付宝接口异常');
        }
        $iv=null;
        ...
        if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
            ...
            if ($this->checkEmpty($apiParams['biz_content'])) {
                weuiMsg('warn-primary', "api请求失败！原因：不支持加密请求！", '支付宝接口异常');
            }
            if ($this->checkEmpty($this->encryptKey) || $this->checkEmpty($this->encryptType)) {
                weuiMsg('warn-primary', "encryptType和encryptKey不能为空！", '支付宝接口异常');
            }
            if ("AES" != $this->encryptType) {
                weuiMsg('warn-primary', "加密类型只支持AES", '支付宝接口异常');
            }
            // 执行加密
