...
class WxPayApi
{
	...
	public static function refund($config, $inputObj, $timeOut = 6)
	{
		...
		$xml = $inputObj->ToXml();
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($config, $xml, $url, true, $timeOut,"refund");
		if($response['code']!=0){
		    return ['return_code'=>1,"return_msg"=>$response['msg']];
        }
		$result = WxPayResults::Init($config, $response['data']);
		self::reportCostTime($config, $url, $startTimeStamp, $result);//上报请求花费时间
	 ...
	 * @throws WxPayException
	 */
	private static function postXmlCurl($config, $xml, $url, $useCert = false, $second = 30,$fun="")
	{
		$ch = curl_init();
		...
		if($useCert == true){
            $sslCert = "";
            $sslKey = "";
            $config->GetSSLCertPath($sslCert, $sslKey);

		    $s = $config->GetMerchantId()."_".md5(createTradeNo());
            $apiclient_cert_path = PAY_PATH . "wxpay/cert/".$s."_apiclient_cert.pem";
            $apiclient_key_path = PAY_PATH . "wxpay/cert/".$s."_apiclient_key.pem";
            if(!is_dir(PAY_PATH . "wxpay/cert/")){
                mkdir(PAY_PATH . "wxpay/cert/",0777);
            }
            ...
            if(($file1=fopen ($apiclient_cert_path,"w+")) === FALSE){
                if($fun=="refund"){
                    ...
                    throw new WxPayException("apiclient_cert证书创建失败，请稍后重试！");
                }
            }
            if(($file2=fopen ($apiclient_key_path,"w+")) === FALSE){
                ...
                if($fun=="refund"){
                    ...
                    throw new WxPayException("apiclient_key证书创建失败，请稍后重试！");
                }
            }
		...
		if($data){
			...
			curl_close($ch);
            if($fun == "refund"){
                return ['code'=>1,"msg"=>"curl出错，错误码:$error"];
            }else{
                throw new WxPayException("对账单接口中，缺少必填参数bill_date！");
            }
		}
